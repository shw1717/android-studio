...
class CivilActivity : AppCompatActivity() {
    ...
    private fun fetchData(safetyCategory: String) {
        ...
        val call = apiService.getSafetyInfo(
            endpoint = "DSSP-IF-20591",
            serviceKey = "RL80RO0LUS2C02O4",
            pageNo = 1,
            numOfRows = 30,
            returnType = "json",
        ...
        call.enqueue(object : Callback<ApiResponse> {
            override fun onResponse(call: Call<ApiResponse>, response: Response<ApiResponse>) {
                if (response.isSuccessful) {
                    ...
                    // 어댑터 초기화 - 드롭다운용
                    val recyclerView: RecyclerView = findViewById(R.id.recyclerView4)
                    recyclerView.layoutManager = LinearLayoutManager(this@CivilActivity)
                    recyclerView.adapter = MultiViewTypeAdapter(groupedItems)
                }
            }

        override fun onFailure(call: Call<ApiResponse>, t: Throwable) {
                Toast.makeText(this@CivilActivity, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
