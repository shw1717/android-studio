...
class NaturalActivity : AppCompatActivity() {
    ...
    private fun fetchData(safetyCategory: String) {
        ...
        val call = apiService.getSafetyInfo(
            endpoint = "DSSP-IF-20588",
            serviceKey = "H13GO7K1AANR3OEL",
            pageNo = 1,
            numOfRows = 30,
            returnType = "json",
            safetyCate = safetyCategory
        )
        ...
        call.enqueue(object : Callback<ApiResponse> {
            override fun onResponse(call: Call<ApiResponse>, response: Response<ApiResponse>) {
                if (response.isSuccessful) {
                    ...
                    // 어댑터 초기화 - 드롭다운용
                    val recyclerView: RecyclerView = findViewById(R.id.recyclerView1)
                    recyclerView.layoutManager = LinearLayoutManager(this@NaturalActivity)
                    recyclerView.adapter = MultiViewTypeAdapter(groupedItems)
                }
            }

            override fun onFailure(call: Call<ApiResponse>, t: Throwable) {
                Toast.makeText(this@NaturalActivity, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
